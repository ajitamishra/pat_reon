...
//       const deployedNetwork = series.networks[networkId];
//       this.series = new web3.eth.Contract(
//         series.abi,
...
//   //   const { getBalance } = this.series.methods;
...
// //     const { sendCoin } = this.series.methods;
...
//             // we load the minimum publication period
            ...
            $('#pledgePerEpisodeEth').html("" + web3.utils.fromWei(pledgePerEpisode, "ether"));
//             // ...and to the support modal

             $('#supportEth').val(web3.utils.fromWei(pledgePerEpisode, "ether"));
            $('#supportPledgePerEpisode').html("" + web3.utils.fromWei(pledgePerEpisode, "ether"));
//             // and then we convert this amount in ETH into fiat currency (here, euros)
            App.getEtherPrice('EUR', function (price) {
                ...
                $('#pledgePerEpisodeFiat').html("" + (price * web3.utils.fromWei(pledgePerEpisode, "ether")).toFixed(2) + ' €');
            });
...
//             // we show that piece of information in the main UI
            $('#nextEpisodePayEth').html('' + web3.utils.fromWei(nextEpisodePay, "ether"));
//             // and we convert it into fiat currency (here euros) to show it as well
            App.getEtherPrice('EUR', function (price) {
                $('#nextEpisodePayFiat').html("" + (price * web3.utils.fromWei(nextEpisodePay, "ether")).toFixed(2) + ' €')
            });
...
//                 // if at least one episode has already been published, we will need 2 pieces of info from the contract: last publication block and minimum publication period
    ...
    refreshSupporterData: function(series, account) {
        ...
        series.pledgePerEpisode().then(function (pledgePerEpisode) {
            ...
            App.getEtherPrice('EUR', function (price) {
                let pledgeFiat = '' + (price * web3.utils.fromWei(pledge, "ether")).toFixed(2) + " €";
...
//      */
    watchEvents: function (account) {
        Series.deployed().then(function (instance) {
            ...
            //     App.refresh(account);
            // });
            instance.events.NewPledge( {},{} )
            .on('data', function(event){
                console.log(event); // same results as the optional callback above
            })
            // instance.events.NewPledge({}, {}).on('data', function(event){
            //     console.log(event);
            //     App.refresh(account);
            // });
    ...
    supportEpisodesChanged: function () {
        ...
        if (supportEpisodesField.val()) {
            ...
            Series.deployed().then(function (instance) {
                ...
                $('#supportEth').val(web3.utils.fromWei(pledgePerEpisode, "ether") * supportEpisodesField.val());
            });
    ...
    supportEthChanged: function () {
        ...
        if (supportEthField.val() && $.isNumeric(supportEthField.val()) && supportEthField.val() > 0) {
            ...
            Series.deployed().then(function (instance) {
                ...
                let numberOfEpisodes = Math.floor(supportEthField.val() / web3.utils.fromWei(pledgePerEpisode, "ether"));
            ...
            web3.eth.getAccounts(function (error, accounts) {
                ...
                if (accounts && accounts.length > 0) {
                    Series.deployed().then(function (instance) {
                        ...
                        let value = web3.utils.toWei(pledgeValue, "ether");
